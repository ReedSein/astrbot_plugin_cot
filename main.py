...
class IntelligentRetryWithCoT(Star):
    ...
    async def store_llm_request(self, event: AstrMessageEvent, req):
        ...
        # --- 新增诊断日志 ---
        logger.debug(f"[IntelligentRetry] 正在存储请求. Key: '{request_key}', Message: '{event.message_str[:50]}...'")
        ...
        # logger.debug(f"已存储LLM请求参数（含完整人格信息和sender信息）: {request_key}") # 您原来的日志
    ...
    def _is_cot_structure_incomplete(self, text: str) -> bool:
        ...
        if not has_os_tag_start and not has_final_reply_tag:
            return False
        if not (has_os_tag_start and has_os_tag_end and has_final_reply_tag):
            logger.debug(f"检测到罗莎CoT结构不完整: {text[:100]}...")
            return True
        return False
    ...
    def _split_and_format_cot(self, response: LLMResponse):
        ...
        if thought_part: log_thought(thought_part)
    ...
    async def process_and_retry_on_llm_response(self, event: AstrMessageEvent, resp: LLMResponse):
        ...
        # --- 新增诊断日志 ---
        logger.debug(f"[IntelligentRetry] 正在处理响应. Key: '{request_key}', Message: '{event.message_str[:50]}...'")

        if request_key not in self.pending_requests:
            # --- 新增详细诊断日志 ---
            logger.warning(f"[IntelligentRetry] Key '{request_key}' 未在 pending_requests 中找到! 插件逻辑将被跳过。")
            logger.debug(f"[IntelligentRetry] 当前待处理的 Keys: {list(self.pending_requests.keys())}")
            return
